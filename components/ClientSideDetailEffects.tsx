...
export default function ClientSideDetailEffects({
  children,
}: ClientSideDetailEffectsProps) {
    ...
    >
      ...
      <div className="absolute top-[10px] left-[25%] right-[25%] h-px flex justify-between px-6 z-50 opacity-30">
        {[...Array(4)].map((_, i) => (
          <div
            key={`t-${i}`}
            className="w-[1.5px] h-[1.5px] rounded-full bg-white shadow-[0_0_5px_white] animate-marquee-pulse"
...
export function AnimatedQuestionCard({
  ...
  return (
    <motion.div
      ...
      className={`transition-all duration-500 will-change-transform ${className || ""}`}
      {...props}
    >
      {children}
    </motion.div>
...
export function AnimatedStatCard({
  ...
  return (
    <motion.div
      ...
      transition={{ delay }}
      className="group flex flex-col items-center justify-center p-6 md:p-8 rounded-4xl bg-white/2 border border-white/5 backdrop-blur-md shadow-sm hover:bg-white/4 hover:shadow-md hover:border-white/8 transition-all duration-700 w-full relative overflow-hidden"
    >
      <div className="absolute inset-x-0 top-0 h-px bg-linear-to-r from-transparent via-white/10 to-transparent pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
      <span className="text-[9px] md:text-[10px] text-white/40 font-bold uppercase tracking-[0.25em] mb-3 text-center z-10">
        {label}
      </span>
      <span
        className={`text-2xl sm:text-3xl md:text-4xl font-light tracking-tight ${color} z-10 drop-shadow-sm`}
      >
        {value}
      </span>
    </motion.div>
