...
export default function QuizEditor({ item, lang }: QuizEditorProps) {
    ...
    const initialQuestions = (item.questions || []).map((q: any) => {
        ...
        supportedLangs.forEach(l => {
            ...
            const answers = q.options?.map((o: any) => o.translations[l]?.text || '') || [];
            translations[l] = {
        ...
        return {
            id: q.id,
            type: q.type,
            mediaUrl: q.videoUrl || q.imageUrl || '',
            stopTime: q.stopTime,
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const questionsDto = formData.levels[0].questions.map((q: any) => {
                ...
                const maxAnswers = Math.max(...formData.languages.map(l => (q.translations[l]?.answers || []).length));

                const options = [];
                for (let i = 0; i < maxAnswers; i++) {
                    const optionTranslations = formData.languages.map(l => ({
                        language: l,
        ...
        <div className="bg-background min-h-screen text-foreground p-8">
            <div className="max-w-4xl mx-auto space-y-8">
                <div className="flex justify-between items-center">
                    <h1 className="text-3xl font-black">{t(lang as any, 'editQuiz') || 'Edit Quiz'}</h1>
