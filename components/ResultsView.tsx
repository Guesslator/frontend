...
export default function ResultsView({ score, totalQuestions, contentId, slug, lang }: ResultsViewProps) {
    ...
    // Theme logic
    const theme = percentage >= 90 ? {
        gradient: 'from-yellow-500/20 via-black to-black',
        textColor: 'text-yellow-400',
        borderColor: 'border-yellow-500/30',
        title: validLang === 'tr' ? 'MÜKEMMEL!' : validLang === 'de' ? 'PERFEKT!' : 'PERFECT!'
    } : percentage >= 70 ? {
        gradient: 'from-gray-400/20 via-black to-black',
        textColor: 'text-gray-300',
        borderColor: 'border-gray-500/30',
        title: validLang === 'tr' ? 'HARIKA!' : validLang === 'de' ? 'GROSSARTIG!' : 'GREAT!'
    } : percentage >= 50 ? {
        gradient: 'from-orange-600/20 via-black to-black',
        textColor: 'text-orange-400',
        borderColor: 'border-orange-500/30',
        title: validLang === 'tr' ? 'İYİ!' : validLang === 'de' ? 'GUT!' : 'GOOD!'
    } : {
        gradient: 'from-red-900/20 via-black to-black',
        textColor: 'text-red-400',
        borderColor: 'border-red-500/30',
        title: validLang === 'tr' ? 'TEKRAR DENE!' : validLang === 'de' ? 'NOCHMAL!' : 'TRY AGAIN!'
    };
    ...
    return (
        <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-4 relative overflow-hidden">
            <div className={`absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] ${theme.gradient} -z-10`} />
            ...
            >
                ...
                <div className={`text-7xl md:text-8xl font-bold mb-4 ${theme.textColor} drop-shadow-[0_0_30px_rgba(255,255,255,0.2)]`}>
                    {percentage}%
                </div>

                <div className="space-y-2 mb-8">
                    <p className="text-neutral-300 text-lg">
                        <span className="text-green-400 font-bold">{score}</span> {validLang === 'tr' ? 'Doğru' : validLang === 'de' ? 'Richtig' : 'Correct'} • <span className="text-red-400 font-bold">{incorrect}</span> {validLang === 'tr' ? 'Yanlış' : validLang === 'de' ? 'Falsch' : 'Incorrect'}
                    </p>
                </div>
                ...
                {!isSubmitted ? (
                    ...
                    >
                        <h3 className="text-xl font-bold mb-4 text-white">
                            {validLang === 'tr' ? 'Skorunu Kaydet' : validLang === 'de' ? 'Punktestand speichern' : 'Save Your Score'}
                        </h3>
                        <div className="flex gap-2">
                            <input
                                type="text"
                                placeholder={validLang === 'tr' ? 'Adın / Takma Adın' : validLang === 'de' ? 'Dein Name' : 'Your Name / Nickname'}
                                value={name}
                                ...
                                {topScores.map((s, idx) => (
                                    <div key={s.id} className="flex justify-between items-center py-2 border-b border-white/5 last:border-0">
                                        <div className="flex items-center gap-3">
                                            <span className={`font-mono w-6 text-neutral-500 ${idx < 3 ? 'text-yellow-400 font-bold' : ''}`}>
                                                #{idx + 1}
                                            </span>
                                            <span className="font-medium text-neutral-200">{s.guestName}</span>
                                        </div>
                                        <span className="font-bold text-white">{s.score} pts</span>
                                    </div>
                                ))}
                                {topScores.length === 0 && (
                                    <p className="text-neutral-500 text-sm text-center py-4">No scores yet. Be the first!</p>
                                )}
                            </div>
                        </div>
                    </motion.div>
