...
export default function Step3Questions({ lang, formData, setFormData }: Step3QuestionsProps) {
    ...
    const addQuestion = () => {
        ...
        newLevels[levelIndex].questions.push({
            ...
            translations: { en: { question: '', answers: [''] }, tr: { question: '', answers: [''] }, ar: { question: '', answers: [''] } },
            duration: 15
        });
    ...
    const updateQuestionTranslation = (qIndex: number, qLang: string, field: 'question' | 'answer', value: string) => {
        const newLevels = [...formData.levels];
    ...
    const derivedBannerUrl = questions.find((q: any) => q.type === 'VIDEO' && q.mediaUrl)?.mediaUrl
        ? `https://img.youtube.com/vi/${questions.find((q: any) => q.type === 'VIDEO' && q.mediaUrl).mediaUrl.split('v=')[1]?.split('&')[0]}/hqdefault.jpg`
            ...
            <div className="bg-card p-6 rounded-xl border border-border flex items-center gap-6">
                <div className="w-[120px] aspect-[2/3] bg-muted rounded-lg overflow-hidden border border-border shadow-sm flex-shrink-0 relative group">
                    <img
                        ...
                        onError={(e) => (e.target as HTMLImageElement).src = '/placeholder-banner.jpg'}
                    />
                    <div className="absolute inset-0 bg-black/10 group-hover:bg-black/0 transition-colors" />
                </div>
                <div>
                    <h3 className="text-lg font-bold text-foreground mb-1">{t(lang, 'bannerPreview') || 'Banner Preview'}</h3>
                ...
                <div key={q.id} className="bg-card p-6 rounded-xl border border-border">
                    ...
                    {q.type === 'VIDEO' && (
                        <div className="mb-6 space-y-4">
                            ...
                            <p className="text-[10px] text-blue-500 mb-2">
                                * The thumbnail of the first video will be used as the Quiz Banner.
                            </p>
                            ...
                            <p className="text-[10px] text-muted-foreground mt-1">
                                Only YouTube videos are supported. Please paste the full video URL.
                            </p>
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        ...
                        {formData.languages.map((l: string) => (
                            <div key={l} className="space-y-3 p-4 bg-muted/20 rounded-lg border border-border">
                                <div className="flex items-center gap-2 mb-2">
                                    <span className="text-xs font-bold text-muted-foreground bg-muted px-2 py-1 rounded">{l.toUpperCase()}</span>
                                </div>
                                ...
                                <div className="space-y-2 mt-2">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-[10px] text-muted-foreground uppercase tracking-wider">{t(lang, 'correctAnswer')}</span>
                                    </div>
                                    ...
                                    {(q.translations[l]?.answers || ['']).map((ans: string, ansIndex: number) => (
                                        <div key={ansIndex} className="flex gap-2 items-center">
                                            ...
                                            <div className="flex items-center gap-1">
                                                <button
                                                    onClick={() => updateQuestion(qIndex, 'correctAnswerIndex', ansIndex)}
                                                ...
                                                >
                                                    {q.correctAnswerIndex === ansIndex && <div className="w-2 h-2 bg-green-500 rounded-full" />}
                                                </button>
                                                ...
                                                {((q.translations[l]?.answers || ['']).length > 1) && (
                                                    <button
                                                        onClick={() => {
                                                            ...
                                                            if (newCorrectIndex === ansIndex) newCorrectIndex = 0;
                                                            else if (newCorrectIndex > ansIndex) newCorrectIndex--;
                                    ...
                                    >
                                        <Plus size={12} /> {t(lang, 'addQuestion').replace('Question', 'Answer')}
                                    </button>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="mt-6 grid grid-cols-3 gap-4 p-4 bg-muted/30 rounded-lg border border-border">
                        <div>
                            ...
                            <input
                                ...
                                value={q.startTime || 0}
                                onChange={e => updateQuestion(qIndex, 'startTime', parseInt(e.target.value))}
                            />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">
                            ...
                            <input
                                ...
                                className="w-full bg-background border border-input rounded-lg px-4 py-2 text-sm text-center focus:border-primary focus:outline-none text-foreground transition-colors"
                                value={q.stopTime || 0}
                                onChange={e => updateQuestion(qIndex, 'stopTime', parseInt(e.target.value))}
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="w-full bg-background border border-input rounded-lg px-4 py-2 text-sm text-center focus:border-primary focus:outline-none text-foreground transition-colors"
                                value={q.endTime || 0}
                                onChange={e => updateQuestion(qIndex, 'endTime', parseInt(e.target.value))}
                            />
                        </div>
                    </div>
                </div>
